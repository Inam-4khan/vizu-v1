...
export const EditProfilePage: React.FC<EditProfilePageProps> = ({ user, onBack, onSave, isGhostMode }) => {
    ...
    <div className={`min-h-full flex flex-col transition-colors duration-500 ${isGhostMode ? 'bg-[#0A001F] text-white' : 'bg-[var(--app-bg,#FFF9E6)] text-[var(--text-primary,#0B1720)]'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div className="relative h-40 rounded-[2rem] overflow-hidden group" onClick={handleBannerClick}>
          <img src={banner} className={`w-full h-full object-cover transition-all duration-500 ${isGhostMode ? 'opacity-50 grayscale' : ''} ${isUploadingBanner ? 'scale-110 blur-sm' : ''}`} alt="Banner" />
        ...
        <div className="flex justify-center -mt-16 relative z-10">
          <div className="relative group" onClick={handleAvatarClick}>
            <div className={`w-32 h-32 rounded-[2.5rem] p-1 transition-all duration-500 ${isGhostMode ? 'bg-[#0A001F]' : 'bg-[var(--app-bg,#FFF9E6)]'} ${isUploadingAvatar ? 'scale-95' : ''}`}>
              <img src={avatar} className={`w-full h-full rounded-[2.3rem] object-cover transition-all duration-500 ${isUploadingAvatar ? 'brightness-50 blur-[2px]' : ''}`} alt="Avatar" />
            </div>
