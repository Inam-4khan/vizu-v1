...
export const HushPage: React.FC<HushPageProps> = ({ isGhostMode, onCameraOpen, notes, onAddNote }) => {
  ...
  const handleCreateNote = () => {
    ...
    const newNote: HushNote = {
      ...
      text: noteText,
      music: noteMusicTitle ? { title: noteMusicTitle, artist: noteMusicArtist || 'Unknown Persona' } : undefined,
      timestamp: 'Just now',
      expiresAt,
    ...
    <div className={`min-h-full transition-all duration-700 pb-24 px-4 ${isGhostMode ? 'bg-[#03171C] text-[#F1FAEE]' : 'bg-[var(--app-bg,#FFF9E6)] text-[var(--text-primary,#0B1720)]'}`}>
      ...
      {isVaultOpen && (
        <div className="px-4 pt-8 animate-fade-in">
           ...
           <div className="space-y-3">
              {[MOCK_USERS[1], MOCK_USERS[2]].map(user => (
                <div 
                  key={`hidden-${user.id}`}
                  onClick={() => setSelectedUser(user)}
                  className={`p-4 rounded-[2.5rem] flex items-center gap-4 transition-all cursor-pointer shadow-lg border ${isGhostMode ? 'bg-[#062B34] border-[#2EC4B6]/30 hover:bg-[#0C3B46] shadow-[#2EC4B6]/10' : 'bg-white border-black/5 hover:border-primary/10'}`}
                >
                   <div className="w-12 h-12 rounded-2xl border overflow-hidden relative">
                      <img src={user.avatar} className={`w-full h-full object-cover ${isGhostMode ? 'grayscale brightness-125' : ''}`} alt="" />
                   ...
                   <div className={`p-2 rounded-xl ${isGhostMode ? 'bg-[#2EC4B6]/20 text-[#80FFEC]' : 'bg-primary/5 text-primary/40'}`}>
                    <Archive size={16} />
                   </div>
                </div>
              ))}
      ...
      <div className="p-4 space-y-4">
        ...
        {MOCK_USERS.map((user) => (
          <div 
            key={user.id} 
            onClick={() => setSelectedUser(user)}
            className={`p-4 rounded-[2.5rem] flex items-center gap-4 border shadow-sm active:scale-[0.98] transition-all cursor-pointer group ${
              isGhostMode 
          ...
          >
            <div className="relative">
              <img src={user.avatar} className={`w-14 h-14 rounded-2xl border-2 transition-colors group-hover:border-[#2EC4B6] ${isGhostMode ? 'border-[#03171C]' : 'border-[var(--app-bg,#FFF9E6)]'}`} alt="" />
      ...
      {isNoteModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-6 bg-black/60 backdrop-blur-md animate-fade-in">
          <div 
            onTouchStart={(e) => setModalStartY(e.touches[0].clientY)}
            onTouchMove={(e) => {
              if (modalStartY !== null) {
                setModalCurrentY(e.touches[0].clientY);
              }
            }}
            onTouchEnd={() => {
        ...
        <div className="fixed inset-0 z-[220] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
          <div className={`w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl animate-scale-up border transition-colors ${
            ...
            <div className={`p-3.5 rounded-2xl border mb-4 flex items-center gap-3 ${
              ...
              <img src={quickReplyNote.avatar} className="w-10 h-10 rounded-xl object-cover shrink-0 border border-secondary/20" alt="" />
      ...
      <header className="p-6 text-white flex items-center justify-between shadow-xl transition-colors duration-500 bg-primary">
         <div className="flex items-center gap-4">
           ...
           <div className="flex items-center gap-3">
              <img src={user.avatar} className="w-10 h-10 rounded-xl border border-white/20" alt="" />
              <div>
              ...
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                 <div className="p-4 rounded-[2rem] max-w-[85%] border shadow-xl flex items-center gap-4 bg-white border-black/5 text-primary">
                    <div className="relative group">
                       <img src={msg.music.albumArt} className="w-16 h-16 rounded-2xl shadow-lg group-hover:brightness-75 transition-all" alt="" />
                       <button 
