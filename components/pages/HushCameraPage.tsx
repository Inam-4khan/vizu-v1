...
export const HushCameraPage: React.FC<HushCameraPageProps> = ({ onBack, isGhostMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col max-w-md mx-auto h-full w-full relative">
      <div className="relative flex-1 overflow-hidden bg-black rounded-b-[2.5rem]">
        ...
        <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-10 safe-area-inset-top">
           <button onClick={onBack} className="p-2 rounded-full bg-black/40 text-white backdrop-blur-md">
             <ArrowLeft size={24} />
           </button>
        </div>
      ...
      {/* Recipient Slider */}
      <div className={`fixed bottom-0 left-0 w-full bg-white rounded-t-[2.5rem] transition-transform duration-500 ease-out z-20 flex flex-col max-h-[80vh] max-w-md mx-auto ${showRecipients ? 'translate-y-0' : 'translate-y-full'}`}>
         ...
         <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {MOCK_USERS.map(user => (
              <div 
                ...
                className={`flex items-center gap-4 p-3 rounded-2xl border transition-all cursor-pointer ${selectedRecipients.includes(user.id) ? 'bg-secondary/10 border-secondary' : 'bg-white border-gray-100'}`}
              >
