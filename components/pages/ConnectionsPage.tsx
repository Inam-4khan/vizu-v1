...
export const ConnectionsPage: React.FC<ConnectionsPageProps> = ({ onBack, onViewProfile, isGhostMode }) => {
    ...
    <div className={`min-h-full flex flex-col transition-colors duration-500 ${isGhostMode ? 'bg-[#0A001F] text-white' : 'bg-[var(--app-bg,#FFF9E6)] text-[var(--text-primary,#0B1720)]'}`}>
      ...
      <div className="p-4 space-y-3 flex-1 overflow-y-auto no-scrollbar">
        {filteredList.map((user) => (
          ...
          >
            <div className="relative">
              <img src={user.avatar} className={`w-14 h-14 rounded-2xl border-2 ${isGhostMode ? 'border-purple-500/20 grayscale opacity-70' : 'border-[var(--app-bg,#FFF9E6)]'}`} alt="" />
